#include "experiment.h"
  ...
  Experiment::Experiment(int nNodes, int sTime, double txGen, 
  ...
  this->speed = speed;
	...
	cout << " --sTime = " << sTime << " --txGen = " << txGen;
  cout << " --mMobility = " << mMobility << " --mLoss = " << mLoss;
  cout  <<  " --nScen = " << nScen << " --speed = " << speed << endl;

// Rueles of Simulation
	if (txGen <= 0){
		cout << " txGen must be a possitive number " << endl;
    exit(1);
	}

  if (nNodes > 50){
    cout << " nNodes must be at must 50 " << endl;
    exit(1);
  }

  if (sTime > 7200){
    cout << " sTime must be at must 7200 seconds " << endl;
    exit(1);
  }
  ...
  if (mobilityModel == cPosition){
    ...
    for(int i = 0; i < nNodes; i++){
      if(i % (nNodes/nGroup) == 0){
        ...
        cout << " Installing followrs in container : " << i << endl;
      }
    }
...
void Experiment::CreateApplications(){
  ...
  b4mesh_apps = b4meshHelper.Install(nodes, txGen);
...
void Experiment::CreateMobilityApplication(){
  ...
  //  mobility_apps = b4meshMobility.Install(nodes, sTime, numScenario);
