...
void B4MeshMobility::SetUp(Ptr<Node> node, vector<Ipv4Address> peers, int sTime, int nScen, int mMob, double speed){
  ...
  debug_suffix.str("");
  debug_suffix << "Movility Model : " << this->mMob << " Simulation time: " << this->duration << " Node's speed: " << this->speed << endl;
 ...
 /*
  * Detects changes in the olsr routing table and executed function TableChange
  */
...
void B4MeshMobility::ChangeGroup(vector<pair<int, Ipv4Address>> groupCandidate){
  ...
  GetB4MeshOracle(node->GetId())->ChangeGroup(make_pair(groupId, group)); // Changed because new of new ChangeGroup() inside Oracle 
...
void B4MeshMobility::UpdateFollowersScn3(Vector leader_pos){
  ...
  if(node->GetId() == (unsigned int)onethird){
  ...
  if(node->GetId() == (unsigned int)twothird){
...
Vector B4MeshMobility::UpdateLeaderPos(){
    ...
    if (scenario == 2){
      ...
      else if (Simulator::Now().GetSeconds() > (duration/3)-((duration/3)/2) && Simulator::Now().GetSeconds() < (duration/3)*2-((duration/3)/2) ){
        debug("****** 2/3 *******");
...
Vector B4MeshMobility::UpdateDirection(){
    ...
    debug_suffix.str("");
    debug_suffix << " Updated position in X: " << current_pos.x << " upadted possition in Y: " << current_pos.y << endl;
    debug(debug_suffix.str());
