...
void B4MTraces::StartElection(float timestamp, uint32_t leader){
  if (start_election == -1){
    start_election = timestamp;

    // Pedro:
    cout << timestamp << " S_ELECTION " << leader << " " << timestamp - start_election << endl;

  }
}

void B4MTraces::EndElection(float timestamp, uint32_t leader){
  if (start_election != -1){
    election_delay[start_election] = timestamp - start_election;

    // Pedro:
    cout << timestamp << " E_ELECTION " << leader << " " << timestamp - start_election << endl;
...
void B4MTraces::StartConfigChange(float timestamp, uint32_t leader){
  if (start_config_change == -1) {
    start_config_change = timestamp;

    // Pedro:
    cout << start_config_change << " S_TOPO_CHANGE " << leader << " " << timestamp - start_config_change << endl;

  }
}
void B4MTraces::EndConfigChange(float timestamp, uint32_t leader){
  if (start_config_change != -1){
    config_change_delay[start_config_change] = timestamp - start_config_change;

    // Pedro:
    cout << start_config_change << " E_TOPO_CHANGE " << leader << " " << timestamp - start_config_change << endl;
