...
class B4MeshRaft : public Application{
  ...
  public:
     ...
     */
    void SetUp(Ptr<Node> node, vector<Ipv4Address> peers, int hBeat, int eTimeout);
  ...
  public:
    ...
    /**
     * Retrieve commited blocks from the committed_pool.
     */
    vector<Block> RetrieveBlocks();

    /**
     * Insert an entrie to be published by raft.
     */
    void InsertEntry(Block& b, int term = -1);

    /**
     * Initiate the group change algorithm.
     */
    ...
    /**
     * Leader have a chance to generate a block every 5 seconds.
     */
    void GenerateBlock();
  ...
  private:
    ...
    /**
     * Method to periodicaly send heartbeats.
     */
    void SendHeartbeats();

    /**
     * Method ot process AppendEntries message.
     */
  ...
  private:
     ...
     */
    void SendPacket(ApplicationPacket& packet, Ipv4Address ip, bool scheduled = false, string msg_t = "");
  ...
  private:
    ...
    map<string, vector<pair<int, Ipv4Address>>> groups; // List of all groups
    ...
    map<string, Block> mempool; // Data waiting to be broadcasted on the network
